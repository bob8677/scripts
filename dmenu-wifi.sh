...
LIST=$(nmcli --fields "$FIELDS" device wifi list | sed '/SSID/d;/^--/d' | sort | uniq)
# For some reason rofi always approximates character width 2 short... hmmm
RWIDTH=$((($(echo "$LIST" | head -n 1 | awk '{print length($0); }')*8)+10))
...
if [ "$LINENUM" -gt 15 ] && [[ "$CONSTATE" =~ "enabled" ]]; then
	LINENUM=15
...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | dmenu -p "Wi-Fi SSID: " -l "$LINENUM" -y "$YOFF" -x "$XOFF" -w "$RWIDTH" -n $HIGHLINE)
#echo "$CHENTRY"
